...
def point_source_centroiding(band,sci_img,d2cMaps,spec_grid=None,fit='2D',center=None,offset_slice=0,verbose=True, return_err=False):
    # distortion maps
    ...
    if fit == '1D':
        ...
        failed_fits = []
        alpha_centers2D_err=np.array([np.nan]*len(alpha_centers2D))
        beta_centers2D_err=np.array([np.nan]*len(alpha_centers2D))
        for ibin in range(len(lambcens)):
            ...
            alpha_centers2D_err[ibin]=std_err[1]
            beta_centers2D_err[ibin]=std_err[2]
        ...
        if return_err :
            return sign_amp2D,alpha_centers2D,beta_centers2D,sigma_alpha2D,sigma_beta2D,bkg_amp2D, alpha_centers2D_err, beta_centers2D_err
...
def point_source_centroiding_singleBin(band,sci_img,d2cMaps,spec_grid=None,fit='2D',center=None,ibin=None):
    ...
    if fit == '1D':
        ...
        return sign_amp2D,alpha_centers2D,beta_centers2D,sigma_alpha2D,sigma_beta2D,bkg_amp2D, np.sqrt(np.diag(pcov))
